...
def balance_sel(df:pd.DataFrame, ranseed: int=30): 
    ...
    nr_0 = df[df.is_healthy==0].id.nunique() # patient number of health_0 class
    nr_1 = df[df.is_healthy==1].id.nunique()
    
    major_cls = np.argmax([nr_0,nr_1]) # label of majority class, 0 or 1
    minor_cls = np.argmin([nr_0,nr_1])
    major_nr = max(nr_0,nr_1)
    minor_nr = min(nr_0,nr_1)

    #select random samples from majority class
    random.seed(ranseed)
    ran_idx = random.sample(range(major_nr),minor_nr) 

    # random select patient id from majority class
